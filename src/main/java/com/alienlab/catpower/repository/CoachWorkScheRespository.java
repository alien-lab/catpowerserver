...
public interface CoachWorkScheRespository extends JpaRepository<CoachWorkSche,Long> {
    ...
    @Query("select a from CoachWorkSche a where a.coach=?1 and a.workDate>?2")
    List<CoachWorkSche> findCoachWorkSchesByCoachAndWorkDate(Coach coach,ZonedDateTime date);
}
