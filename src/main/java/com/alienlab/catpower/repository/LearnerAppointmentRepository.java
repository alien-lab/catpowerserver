...
public interface LearnerAppointmentRepository extends JpaRepository<LearnerAppointment,Long> {
    ...
    @Query("select a from LearnerAppointment a where a.buyCourse.learner.id=?1 and a.appointmentDate>=?2 order by a.appointmentDate DESC")
    List<LearnerAppointment> findAppointmentByLearner(Long learnerId,ZonedDateTime date);

    @Query("select a from LearnerAppointment a where a.buyCourse.learner.id=?1 and a.appointmentResult='预约中' and a.appointmentDate>=?2 order by a.appointmentDate DESC")
    List<LearnerAppointment> findAppointingByLearner(Long learnerId,ZonedDateTime date);
